...
- Convince you that you should package data!
  - Presentation (Myself & CORI)
  - FCC's data
...
- Two datasets are providing an overview of broadband access since 2014

- Data is files (csv), tabular format
...
| Releases | twice a year | twice a year[^verizon]                                                                                                                                                                                                               |
| Size | 400MB/year | 22GB/year                                                                                                                                                                                                                            |

[^verizon]: One release can have multiple versions


## FCC data in our apps and analysis work

Broadband data is used extensively in our work to provide critical support to secure funding, develop infrastructure, and implement
<br />the programs needed to unlock the potential of rural America.
...
- Multiple projects at different time, multiple tables
...
- New project / New spec -> copy/pasting and maintaining X **similar** code sources
...
- complexity is **one** place and **abstracted**

- provide a timestamp: solve a bit the "multiple tables" issue
...
<!--
![DuckDB](img/DuckDB_logo.svg)
-->

Compute: PG (RDS) -> DuckDB 

Storage: PG (RDS) -> S3 (Range HTTP, storage with nice access)

added benefits: lower cost, less contraintes on permissions -> share it!

- Parquet and interoperability:
  * language change file format also but way slowly
...
- Open source is :
   - a way to publish: i.e., you can read the code
   - a way to share: i.e., you can use it

We care more about process than product or to say it differently,
the product is just a means to reach a goal (a process).
<br />We want to lower the cost of using that data so rural america
can validate the service claims made by their ISP and manage their
<br />strategy for broadband access and expansion.
